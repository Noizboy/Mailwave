...
function getNextEmailLabel(campaign: CampaignDetail, nowMs: number): string | null {
  ...
  if (campaign.nextSendAt) {
    ...
    return remainingMs > 0 ? `Next email in ${formatCountdown(remainingMs)}` : "Sending now";
  }
  ...
  return fallbackTargetMs > nowMs ? `Next email in ${formatCountdown(fallbackTargetMs - nowMs)}` : "Sending now";
}
...
export function CampaignDetailClient({ campaignId }: { campaignId: string }) {
  ...
  const handleBulkApproval = async (approvalStatus: "approved" | "skipped") => {
    const emailIds = [...selectedIds];
    const res = await fetch(`/api/campaigns/${campaignId}/emails/bulk-status`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (res.ok) {
      ...
      toast.success(
        `${updated} email${updated === 1 ? "" : "s"} ${label}`,
        ""
      );
      setSelectedIds(new Set());
      invalidate();
  ...
  const handleCancelGenerate = async () => {
    ...
    const res = await fetch(`/api/campaigns/${campaignId}/generate/cancel`, { method: "POST" });
    if (res.ok) {
  ...
  const canBulkSelect = !isSending && campaign.status !== "completed" && emails.length > 0;
  const allFilteredSelected = filteredEmails.length > 0 && filteredEmails.every((e) => selectedIds.has(e.id));
  const someFilteredSelected = filteredEmails.some((e) => selectedIds.has(e.id));
    ...
    <div className="flex flex-col h-full">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="space-y-5">
            ...
            <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-5 py-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  The AI service returned an error or no eligible contacts were found. Check your AI settings and try again.
                </p>
              </div>
            </div>
          )}
          ...
          {["sending", "paused", "completed"].includes(campaign.status) && campaign.totalEmails > 0 && (
            <div className="rounded-xl border bg-card px-5 py-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            selectedIds.has(email.id) ? "bg-primary/5" : isSelected ? "bg-accent" : "hover:bg-muted/40"
                          )}
                        >
                          {canBulkSelect && (
                            <div
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => toggleEmailSelection(email.id, checked === true)}
