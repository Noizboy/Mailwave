...
### Security / Correctness
- **Send worker skips unsubscribed and invalid contacts** — The `pendingEmails` query in `send-campaign.ts` previously used `contact: { status: { not: "suppressed" } }`, which allowed contacts marked `unsubscribed` or `invalid` between generation and sending to still receive emails. The filter is now `contact: { status: "subscribed" }`, so only actively subscribed contacts are ever sent to regardless of when their status changed.
